-- Update the create_business_phases_for_project function to create only brand_assets for Phase 2
...
DECLARE
  ...
  phase6_id uuid;
BEGIN
  -- Get team IDs (using Division naming)
  SELECT id INTO research_team_id FROM teams WHERE name = 'Research Division' LIMIT 1;
  SELECT id INTO brand_team_id FROM teams WHERE name = 'Brand & Design Division' LIMIT 1;
  SELECT id INTO dev_team_id FROM teams WHERE name = 'Development Division' LIMIT 1;
  SELECT id INTO content_team_id FROM teams WHERE name = 'Content Division' LIMIT 1;
  SELECT id INTO marketing_team_id FROM teams WHERE name = 'Marketing Division' LIMIT 1;
  SELECT id INTO sales_team_id FROM teams WHERE name = 'Sales Division' LIMIT 1;
  ...
  -- Create deliverables for Phase 2: Brand & Identity (SINGLE brand_assets deliverable)
  INSERT INTO phase_deliverables (phase_id, user_id, name, deliverable_type, description, assigned_team_id) VALUES
    (phase2_id, NEW.user_id, 'Brand Assets', 'brand_assets', 'Generated logos, icons, color palette, and social banners', brand_team_id);
